...
contract FloodHelp{
    ...
    function closeRequest(uint id) public {
        ...
        require(requests[id].open && (msg.sender == author || balance >= requests[id].goal), unicode"Você não pode fechar este pedido");
    ...
    function donate(uint id) public payable {
        require(msg.value > 0, unicode"O valor deve ser maior que 0");
